...
export default async function Home({ searchParams }: Props) {
  ...
  if (sort === "rated") query = query.order("rating", { ascending: false }).order("created_at", { ascending: false });
    ...
    <main className="min-h-screen p-6 max-w-6xl mx-auto">
      <header className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link className="underline text-white" href="/admin">Admin</Link>
          <Link className="underline text-white" href="/login">Login</Link>
        </div>
      </header>
      ...
      <section className="mt-4 border border-zinc-300 bg-white rounded-lg p-4">
        <form className="grid gap-3 md:grid-cols-12" action="/" method="get">
          ...
          <div className="md:col-span-2 flex items-end">
            <label className="flex items-center gap-2 select-none">
              ...
              />
              <span className="text-sm font-medium text-zinc-800">Rated only</span>
            </label>
          </div>
        ...
        {error ? <p className="mt-3 text-sm text-red-600">Error: {error.message}</p> : null}
      </section>

      {/* Results */}
      <section className="mt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {books.map((b) => (
          <article key={b.id} className="border border-zinc-300 bg-white rounded-lg p-3">
            {b.cover_url ? (
            ...
            <div className="mt-2">
              <div className="font-semibold text-zinc-900 line-clamp-2">{b.title}</div>
              <div className="text-sm text-zinc-700 line-clamp-1">{b.author ?? "Unknown"}</div>

              <div className="text-xs mt-2 flex items-center gap-2 flex-wrap">
                <span className="inline-block px-2 py-0.5 rounded bg-zinc-100 border border-zinc-200">
                  {b.status}
                </span>
                {typeof b.rating === "number" ? (
                  <span className="font-medium">⭐ {b.rating}/5</span>
                ) : (
